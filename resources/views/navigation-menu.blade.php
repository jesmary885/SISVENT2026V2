<nav x-data="{ open: false }" class="bg-white border-b border-gray-300">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white shadow-lg">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Ventas -->
            <x-responsive-nav-link href="{{ route('ventas_index') }}" :active="request()->routeIs('ventas_index')">
                <div class="flex items-center">
                    <svg class="w-5 h-5 mr-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<style>
    ...
    .sm\:hidden {
        max-height: calc(100vh - 112px); /* Ajusta según la altura de tu header */
        overflow-y: auto;
    }
    
    /* Estilos para los enlaces del menú responsive */
    .responsive-nav-link {
    ...
    /* Íconos más claros en estado activo */
    .responsive-nav-link.active svg {
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const hamburgerButton = document.querySelector('[x-data="{ open: false }"]');
        
        if (hamburgerButton) {
            const responsiveLinks = document.querySelectorAll('.responsive-nav-link');
            
            responsiveLinks.forEach(link => {
                link.addEventListener('click', function() {
                    // Solo cerrar en móviles
                    if (window.innerWidth < 640) {
                        hamburgerButton.__x.$data.open = false;
                    }
                });
            });
        }
